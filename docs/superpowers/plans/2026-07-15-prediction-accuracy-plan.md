...
- **🔒 버전 격리 (A안 — side-by-side 프리뷰)**: 모든 프론트엔드 변경(Task 5·6·7의 UI 배선)은 **`index-v2.html`(기존 `index.html`의 복사본)에만** 적용한다. **기존 `index.html`은 이 계획 어디에서도 수정하지 않는다** — 매일 쓰는 도구에 무영향. 접속은 기존 URL 뒤에 `-v2.html`(예: `…/rocket-sales-simulator/index-v2.html`). 신버전 확정 후에만 `index-v2.html` → `index.html` 승격(별도 승인). 공유 자산(`js/benchmarks-logic.js`·`benchmarks` 테이블)은 기존 `index.html`이 참조하지 않으므로 신규 추가해도 기존 버전에 영향 없음.
- **줄 번호 주의**: Task 5·6·7에 나오는 `index.html:NNN` 줄 번호는 편집 대상 `index-v2.html`에도 동일하게 적용된다(바이트 복사본이므로). 단, `index-v2.html`을 편집하기 시작하면 줄이 밀리므로 각 Step은 grep 앵커(id·문자열)로 위치를 재확인한 뒤 수정한다.
...
| `index-v2.html` | **기존 `index.html`의 바이트 복사본** — 여기에만 신버전 배선(조회·드롭다운·제안·범위·FT). **기존 `index.html`은 무변경** | 신규(복사) |
...
> ⚠️ **이 Task부터(5·6·7) 모든 편집 대상은 `index-v2.html`이다. 기존 `index.html`은 건드리지 않는다.** (Global Constraints 버전 격리)

**Files:**
- Create: `index-v2.html` (기존 `index.html`의 바이트 복사본)
- Modify: `index-v2.html`만 — 스크립트 include([index.html:1017](../../index.html#L1017) 위치), 전환율 카드([index.html:691-716](../../index.html#L691-L716) 위치), 인라인 JS(전역 + `loadScenariosFromSupabase` 근처 [index.html:1473](../../index.html#L1473) 위치)
...
기존 도구를 손대지 않기 위해 신버전 작업용 복사본을 만든다:

```bash
cd ~/ai-project/work/rocket-sales-simulator
cp index.html index-v2.html
# 버전 배지 등 신버전 식별 표기(선택): index-v2.html 헤더에 "미리보기(v2)" 뱃지 추가 가능
git add index-v2.html
git commit -m "chore(sim): 신버전 프리뷰용 index-v2.html 복사본 생성(기존 index.html 무변경)"
```
이후 Step은 모두 `index-v2.html`을 편집한다.

- [ ] **Step 1: 순수 로직 스크립트 include 추가**

`index-v2.html`의 supabase CDN `<script>`(원본 [1017](../../index.html#L1017)) 바로 다음 줄에 추가 (grep `grep -n 'supabase-js' index-v2.html`로 위치 확인):
...
`index-v2.html` 전환율 카드(원본 [691-716](../../index.html#L691-L716))의 `<div class="grid-2">` 바로 위(카드 타이틀 다음)에 삽입:
...
`index-v2.html`에서 광고비·CAC·오거닉 입력의 `oninput`을 유료비중 변화 시 CVR 재제안이 되도록 확장한다. 세 라인을 각각 아래로 수정:
...
`index-v2.html` 인라인 JS에 전역과 함수를 추가(예: `loadScenariosFromSupabase` 정의 원본 [1473](../../index.html#L1473) 위):
...
`index-v2.html`의 `loadScenariosFromSupabase()` 본문 마지막(`renderScenarios();` 다음, 원본 [1490 부근](../../index.html#L1490))에 추가:
...
`webapp-testing` 스킬 또는 `.dev/sim_capture.py` 하니스로 **`index-v2.html`을** 로컬 서빙 후 확인:
...
cd ~/ai-project/work/rocket-sales-simulator
git add index-v2.html
git commit -m "feat(sim): [v2] 업종 드롭다운 + benchmarks 기반 참석률·결제CVR 자동 제안(유료비중 구간)"
```
...
**Files:**
- Modify: `index-v2.html`만 — 결과 카드 영역 + `calcSim()` 결제자/거래액 렌더(원본 [index.html:1244-1252](../../index.html#L1244-L1252), [1133](../../index.html#L1133))
...
cd ~/ai-project/work/rocket-sales-simulator
git add index-v2.html
git commit -m "feat(sim): [v2] 결과 화면 범위(밴드) 토글 — 결제자·거래액 95% 범위 표시"
```
...
**Files:**
- Modify: `index-v2.html` — FT 참석률 라벨(원본 [index.html:702](../../index.html#L702) 위치)
- Modify: `CLAUDE.md` — "카테고리 분류 체계" 절 (버전 무관 공용 문서)

**Interfaces:** 없음(문서·라벨).

- [ ] **Step 1: FT 참석률 라벨에 "실측 없음" 안내 추가**

`index-v2.html`의 FT 참석률 라벨(원본 [702](../../index.html#L702), grep `grep -n 'FT 참석률' index-v2.html`)을 아래로 교체:
...
```bash
cd ~/ai-project/work/rocket-sales-simulator
git add index-v2.html CLAUDE.md
git commit -m "docs(sim): [v2] FT 실측없음 라벨 + CLAUDE.md 업종 어휘 정합(webinars 정본)"
```
...
`index-v2.html`을 로컬 서빙 → 시나리오 탭에서 기존 저장 시나리오 3~5개를 로드(원본 [index.html:1672-1675](../../index.html#L1672-L1675)에서 s-attend/s-ft/s-cvr 세팅됨).
확인: 로드 시 **업종 드롭다운은 미선택 상태**이고, 거래액·결제자·정산금액 등 결과값이 v2에서도 기존 `index.html`과 **완전히 동일**(계산식·입력 불변). 업종 자동 제안이 로드된 시나리오 값을 덮어쓰지 않아야 함(드롭다운 미선택이므로 `suggestFromBenchmark`는 early-return).
추가 확인: **기존 `index.html`은 이번 작업에서 한 번도 수정되지 않았다** — `git log --oneline -- index.html` 이번 세션 커밋 0건, `git diff <작업시작 커밋> -- index.html` 빈 출력.

Expected: 23건 모두 이전과 동일 결과 + 기존 파일 무변경. (spec 성공기준 §5)
...
- [ ] **Step 6: 프리뷰 배포 (기존 URL 무변경)**

`index-v2.html`을 GitHub Pages에 올려 **기존 URL과 나란히** 접근 가능하게 한다. main push = 배포지만, **기존 `index.html`은 변경 커밋이 없으므로 매일 쓰는 URL은 그대로**다. 신버전 접속: `https://fancy-rgb.github.io/rocket-sales-simulator/index-v2.html`.

🚦 배포 브리핑:
- 뭐가 바뀌나: 신버전 `index-v2.html` 신규 추가 (기존 `index.html` 무변경)
- 누가 영향받나: 아무도 자동 영향 없음 — 신버전은 `-v2.html`로 들어가야만 보임
- 되돌릴 수 있나: 1분 내 (파일 삭제 후 재배포)
- 진행 방식: push 전 **사용자 승인**(외부 반영)

승인 후 push. 배포 후 신버전 URL에서 로그인·업종 제안·범위 토글이 뜨는지 1회 확인.

- [ ] **Step 7: 체험 → 개선안 수집 → 승격 결정**

사용자가 며칠간 `index-v2.html`을 실제로 써보게 한다. 그동안:
1. 개선점·불편·버그를 `docs/v2-feedback.md`에 목록화(원본 전체 보존, "이유" 컬럼 포함)
2. 반영 필요분은 해당 Task로 되돌아가 수정 → 재배포(프리뷰)
3. 사용자가 "이대로 좋다" 확정하면 **승격**: `index-v2.html` → `index.html` 교체

승격 절차(확정 후, 별도 승인):
```bash
cd ~/ai-project/work/rocket-sales-simulator
cp index-v2.html index.html    # 신버전을 정본으로
# CHANGELOG.md·앱 내 changelog 모달·헤더 버전 배지 3곳 갱신 (CLAUDE.md 릴리즈 규칙)
git add index.html CHANGELOG.md
git commit -m "release(sim): v2 예측 정확도 개선을 정본으로 승격"
# index-v2.html은 유지(다음 실험용) 또는 제거 — 사용자 판단
```
승격도 **외부 반영 → 4줄 브리핑 + 승인 필수**.
